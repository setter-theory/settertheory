...
function printMatchPdfReport(){
  // V150.10: PDF専用6ページ固定レイアウト。
  // 既存データ集計は維持し、A4横・固定改ページで別HTMLを生成する。
  ...
  const reportMeta=`${esc(today)}　${esc(s.myTeam || '自チーム')} vs ${esc(s.oppTeam || '相手')}　/　Set ${esc(s.setNo || '1')}　/　Setter ${reportSetters.map(n=>esc(n+'番 '+getPlayerName(n))).join('・')}`;
  const pageHeader=(title,subtitle='')=>`<header class="pageHeader"><div><div class="pageKicker">SETTER THEORY MATCH REPORT</div><h1>${esc(title)}</h1><p>${subtitle||reportMeta}</p></div><div class="aquilaPdfBadge"><img src="${aquilaIcon}" alt="Aquila"><div>${pdfHeaderIq}</div></div></header>`;
  const pageFooter=(pageNo)=>`<footer class="pageFooter"><span>Setter Theory / Generated by Aquila</span><span>${pageNo} / 6　V150.10</span></footer>`;
  const html=`<!doctype html><html lang="ja"><head><meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Setter Theory Report V150.10</title>
  <style>
    @page{size:A4 landscape;margin:5mm;}
    *{box-sizing:border-box;}
    html,body{margin:0;background:#e5e7eb;color:#111827;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
    ...
    .reportPages{padding:8px 0 20px;}
    .pdfPage{position:relative;width:287mm;min-height:200mm;margin:10px auto;background:#fff;padding:7mm 8mm 11mm;box-shadow:0 8px 26px rgba(15,23,42,.16);overflow:hidden;break-after:page;page-break-after:always;}
    .pdfPage:last-child{break-after:auto;page-break-after:auto;}
    .pageHeader{height:25mm;display:flex;align-items:flex-start;justify-content:space-between;gap:12px;border-bottom:3px solid #f4b63f;padding-bottom:6px;margin-bottom:7px;}
    .pageKicker{font-size:8px;font-weight:950;letter-spacing:.14em;color:#b7791f}.pageHeader h1{margin:1px 0 2px;font-size:21px;letter-spacing:.01em;color:#0f172a}.pageHeader p{margin:0;color:#64748b;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:210mm;}
    .aquilaPdfBadge{display:flex;align-items:center;gap:7px;background:linear-gradient(135deg,#0f172a,#1e3a8a);color:#fff;border-radius:13px;padding:6px 9px;min-width:145px;}
    .aquilaPdfBadge img{width:38px;height:38px;object-fit:contain;border-radius:50%;background:#fff;padding:2px}.aquilaPdfBadge .small{font-size:7px;color:#fbbf24;font-weight:900;letter-spacing:.08em}.aquilaPdfBadge .iqLine{display:flex;align-items:baseline;gap:2px}.aquilaPdfBadge .iqLine b{font-size:21px;line-height:1}.aquilaPdfBadge .iqLine span{font-size:8px;font-weight:900}.aquilaPdfBadge .rank{font-size:7px;font-weight:950;letter-spacing:.06em;color:#bfdbfe}
    .pageFooter{position:absolute;left:8mm;right:8mm;bottom:4mm;border-top:1px solid #cbd5e1;padding-top:3px;color:#64748b;font-size:8px;display:flex;justify-content:space-between;}
    .summary{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin:0 0 8px}.metric{border:1px solid #cbd5e1;border-radius:10px;padding:7px;background:#f8fafc;break-inside:avoid;page-break-inside:avoid}.metric .label{font-size:9px;color:#64748b;font-weight:800}.metric .value{font-size:21px;font-weight:950;color:#0f172a;margin-top:1px}.metric .sub{font-size:8px;color:#64748b;margin-top:1px}
    .section{margin:0 0 7px;break-inside:avoid;page-break-inside:avoid}.section h2{font-size:13px;margin:0 0 4px;color:#0f172a;border-left:4px solid #f4b63f;padding-left:6px}.section h3{margin:0 0 4px;font-size:11px;color:#1e3a8a}
    .twoCol{display:grid;grid-template-columns:1fr 1fr;gap:8px;align-items:start}.threeCol{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;align-items:start}.note{font-size:8px;color:#64748b;line-height:1.35;margin:3px 0 0}
    table{width:100%;border-collapse:collapse;margin:0;font-size:8.4px;table-layout:auto}th,td{border:1px solid #cbd5e1;padding:3px 4px;text-align:left;vertical-align:top}th{background:#e2e8f0;color:#0f172a;font-weight:900}td{background:#fff}.empty{text-align:center;color:#64748b;padding:8px!important}tr{break-inside:avoid;page-break-inside:avoid}
    .pdfLead{display:grid;grid-template-columns:${reportSetters.length>1?'1fr 1fr':'1fr'};gap:8px;align-items:start}.pdfSetterLeadPerson{display:grid;grid-template-columns:1fr 1fr;gap:7px;break-inside:avoid;page-break-inside:avoid}.pdfIqCard,.pdfAdviceCard{border:1px solid #cbd5e1;border-radius:10px;padding:7px;background:linear-gradient(180deg,#f8fafc,#fff);break-inside:avoid;page-break-inside:avoid}.pdfIqCard .title,.pdfAdviceCard .title{font-size:10px;font-weight:950;color:#1e3a8a;margin-bottom:4px}.pdfIqCard .score{font-size:28px;font-weight:1000;color:#2563eb;line-height:1}.pdfIqCard .score small{font-size:10px;color:#64748b}.pdfIqCard .rank{display:inline-block;margin-top:3px;border-radius:999px;padding:3px 7px;background:#0f172a;color:#fbbf24;font-size:7px;font-weight:950}.pdfIqScoreRow{display:grid;grid-template-columns:72px 1fr;gap:6px;align-items:center}.pdfIqCard .setterIqRadar{background:#f8fafc;border:1px solid #dbeafe;border-radius:8px;padding:2px;min-width:0}.pdfIqCard .setterIqRadarTitle{display:none}.pdfIqCard .setterIqRadar svg{display:block;width:100%;height:130px;overflow:hidden}.pdfIqCard .radarGrid polygon,.pdfIqCard .radarGrid line{fill:none;stroke:#94a3b8;stroke-width:1}.pdfIqCard .radarGrid polygon:first-child{stroke:#64748b;stroke-width:1.4}.pdfIqCard .radarData{fill:rgba(37,99,235,.24);stroke:#2563eb;stroke-width:3;stroke-linejoin:round}.pdfIqCard .radarDots circle{fill:#1d4ed8;stroke:#fff;stroke-width:2}.pdfIqCard .radarLabels text{font-size:11px;font-weight:1000;fill:#334155}.pdfIqCard .radarLabels tspan+tspan{font-size:10px;font-weight:1000;fill:#1d4ed8}.pdfAdviceCard ul{margin:0;padding-left:15px;font-size:8px;line-height:1.4}.pdfAdviceCard li+li{margin-top:3px}.pdfAdviceTitle{display:flex;align-items:center;gap:5px}.pdfAdviceTitle img{width:21px;height:21px;object-fit:contain;border-radius:50%;background:#fff;border:1px solid #f4b63f;padding:1px}.pdfAdviceTitle span{font-size:10px;font-weight:950;color:#1e3a8a}
    .setterPdfCard{border:1px solid #93c5fd;border-radius:10px;padding:7px;background:#f8fbff;break-inside:avoid;page-break-inside:avoid}.setterPdfTop{display:flex;justify-content:space-between;gap:7px;font-size:9px}.setterPdfTop b{color:#1d4ed8}.setterPdfBreak{display:grid;grid-template-columns:repeat(5,1fr);gap:3px;margin:5px 0}.setterPdfBreak span{background:#dbeafe;border-radius:5px;padding:3px;text-align:center;font-size:7px}.setterPdfCard ul{margin:3px 0 0;padding-left:15px;font-size:8px;line-height:1.35}
    .pdfRankGrid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;align-items:start}.pdfRankCard{border:1px solid #cbd5e1;border-radius:8px;padding:5px;background:#f8fafc;break-inside:avoid;page-break-inside:avoid}.pdfRankCard h3{margin:0 0 3px;font-size:10px;color:#1e3a8a}.pdfRankCard table{font-size:7.7px}.pdfRankCard th,.pdfRankCard td{padding:2.5px 3px}
    .compact table{font-size:7.8px}.compact th,.compact td{padding:2.5px 3px}.logTable table{font-size:7.2px}.logTable th,.logTable td{padding:2px 3px}
    @media print{html,body{background:#fff!important}.topbar{display:none!important}.reportPages{padding:0}.pdfPage{width:auto;height:200mm;min-height:200mm;margin:0;padding:7mm 8mm 11mm;box-shadow:none;overflow:hidden}.pageHeader{height:25mm}.section,.setterPdfCard,.pdfRankCard,.pdfSetterLeadPerson{break-inside:avoid!important;page-break-inside:avoid!important}}
    @media (max-width:900px){.pdfPage{width:calc(100vw - 16px);min-height:0;overflow:visible}.pageHeader p{white-space:normal}.twoCol,.threeCol,.pdfLead,.pdfSetterLeadPerson,.pdfRankGrid{grid-template-columns:1fr}.pageFooter{position:static;margin-top:10px}}
  </style></head><body>
    <div class="topbar"><b>Setter Theory PDFプレビュー　V150.10</b><div><button class="secondary" onclick="window.close()">← レポートへ戻る</button><button onclick="window.print()">📄 PDF/印刷</button></div></div>
    <main class="reportPages">
      <article class="pdfPage page1">${pageHeader('試合サマリー')}
        <div class="summary">
      ...
      <article class="pdfPage page2">${pageHeader('セッター分析')}
      ...
      <article class="pdfPage page3">${pageHeader('チーム分析')}
        <section class="section compact"><h2>プレー別 成功率・効果率</h2>${table(['項目','本数','成功','ミス','被ブロック','成功率','効果率'], actionRows, '記録がありません。')}</section>
        <section class="section compact"><h2>選手別 成功率・効果率</h2>${table(['選手','名前','本数','成功','ミス','被ブロック','成功率','効果率'], playerRows, '選手別の対象記録がありません。')}</section>
        <div class="twoCol"><section class="section"><h2>ローテーション別</h2>${table(['ローテ','本数','成功','成功率','得点','失点','差'], rotRows, '記録がありません。')}</section><section class="section"><h2>プレー割合</h2>${table(['項目','本数','割合'], playRows, '記録がありません。')}</section></div>
        ${pageFooter(3)}
      </article>
      <article class="pdfPage page4">${pageHeader('トス・二段トス分析')}
        <div class="twoCol"><section class="section"><h2>トス配分</h2>${table(['トス先','本数','割合'], tossRows, 'トス記録がありません。')}</section><section class="section"><h2>トス技術</h2>${table(['総トス','成功','トスミス','成功率','ミス率'], [[`${tossQuality.total}`,`${tossQuality.success}`,`${tossQuality.miss}`,`${tossQuality.successRate}%`,`${tossQuality.missRate}%`]], 'トス記録がありません。')}<div class="note">※トスミスは得点・失点とは別に、トスの技術的な質として記録します。</div></section></div>
        <section class="section"><h2>二段トス分析</h2>${table(['選手','名前','合計','レフト','センター','ライト','バック','ツー'], secondBallRows, '二段トスの記録がありません。')}<div class="note">※二段トスは通常トス・Setter IQとは別集計です。</div></section>
        ${pageFooter(4)}
      </article>
      <article class="pdfPage page5">${pageHeader('各項目ランキング TOP3')}
      ...
      <article class="pdfPage page6">${pageHeader('履歴・直近ログ')}
        <section class="section"><h2>選手交代履歴</h2>${table(['ペア','回数','最終スコア','最終ローテ','最終時刻'], subRows, '選手交代の記録がありません。')}</section>
        <section class="section logTable"><h2>直近ログ</h2>${table(['No','Set','Rot','プレー','選手','結果','得点','スコア','時刻'], recentRows, 'ログがありません。')}</section>
        ${pageFooter(6)}
      </article>
    </main>
  </body></html>`;
