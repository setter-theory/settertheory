...
// V150.207: PDFプレビュー内のAquila画像を埋め込み、Safariの画像読み込み失敗を防ぐ。
...
function save(reason="auto"){
  try{
    ...
    // V150.207: Undo履歴はメモリ上に保持し、端末保存は直近30件だけに制限する。
...
function archiveFinishedCurrentMatch(){
  ...
  const saved=migrateSavedMatchIdentities({
    ...
    // V150.207: プレー明細はcsvに保存済みなので、liveStateには名簿・設定だけを残す。
    // 同じログの二重保存による容量超過を防ぐ。
...
function finishMatch(){
  ...
  try{
    ...
    // V150.207: 保存一覧へ書き込まれた内容を、終了処理前に最終スコア・ログ件数まで照合する。
...
function printMatchPdfReport(){
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    <div class="pdfPreviewBuildMarker">V150.207</div>
      ...
      if(printButton && printButton.dataset.bound!=='1'){
        ...
        printButton.addEventListener('click',async(ev)=>{
          ...
          try{
            ...
            // V150.207: 埋め込み画像を含む全画像のデコード完了後にPDF化する。
...
function setSavedMatches(list){
  // V150.207: 過去の保存試合も書き込み時に軽量化し、容量を回復する。
...
function buildImportedUnifiedReport(parsed){
  ...
  withImportedMatchState(parsed,()=>{
    ...
    // V150.207: 保存した試合→レポートだけに、個人ランクとAquila Adviceを追加する。
